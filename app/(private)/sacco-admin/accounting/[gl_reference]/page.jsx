...
export default function GLAccountReferencePage() {
    ...
    const { data: account, isLoading, error, refetch } = useFetchGLAccount(gl_reference);
    ...
    return (
        <div className="min-h-screen bg-gray-50/50 dark:bg-slate-950 p-4 md:p-6 space-y-6">
            {/* Header */}
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div className="space-y-1">
                    <Link
                        href="/sacco-admin/accounting"
                        className="p-0 hover:bg-transparent text-slate-500 hover:text-slate-900 dark:hover:text-slate-200 flex items-center gap-1 mb-2 group transition-all"
                    >
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        <Badge variant="outline" className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-600 text-slate-500 px-2 py-0 h-6">
                            {account.code}
                        </Badge>
                        <Badge
                            variant="outline"
                            className={`uppercase text-[10px] h-6 ${account.is_active
                    ...
                    <p className="text-sm text-slate-500 font-medium">
                        GL Ledger Reference: <span className="text-slate-900 dark:text-slate-100 font-mono">{account.reference}</span>
                    </p>
                ...
                <Button
                    ...
                    className="bg-[#ea1315] hover:bg-[#c40f11] text-white shrink-0"
                >
                    ...
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                            <div className="space-y-1">
                                ...
                                <h3 className="text-xl text-slate-900 dark:text-slate-100 font-mono">
                                    KES {Number(account.balance).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                </h3>
                            </div>
                            <div className="w-12 h-12 rounded bg-slate-50 dark:bg-slate-800 flex items-center justify-center group-hover:bg-[#ea1315]/10 transition-colors">
                    ...
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                            <div className="space-y-1">
                                ...
                                <h3 className="text-xl text-slate-900 dark:text-slate-100 capitalize">
                                    {account.category?.toLowerCase()}
                                </h3>
                            </div>
                            <div className="w-12 h-12 rounded bg-slate-50 dark:bg-slate-800 flex items-center justify-center group-hover:bg-blue-50 transition-colors">
            ...
            {/* Ledger Table */}
            <Card className="border-none shadow-sm bg-white dark:bg-slate-900 rounded">
                <CardHeader className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 border-b dark:border-slate-700">
                    <div className="space-y-1">
                    ...
                    <div className="w-full md:w-80">
                        <div className="relative">
                            ...
                            <Input
                                ...
                                className="pl-10 h-10 bg-slate-50 dark:bg-slate-800 border-none rounded text-sm placeholder:text-slate-500 focus-visible:ring-1 focus-visible:ring-[#ea1315]"
                            />
                        </div>
                    </div>
                </CardHeader>
                                ...
                                {filteredEntries.map((entry) => (
                                    <TableRow key={entry.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors border-b-slate-100">
                                        ...
                                        <TableCell className="py-4 text-right pr-6 font-mono">
                                            <span className={`text-sm ${entry.runningBalance >= 0 ? "text-slate-900 dark:text-slate-100" : "text-rose-600"}`}>
            ...
            <Dialog open={isAddEntryOpen} onOpenChange={setIsAddEntryOpen}>
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Create a balanced journal batch with <span className="font-semibold">{account.name}</span> pre-selected as the first line.
                        </DialogDescription>
                    </DialogHeader>
